    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
        ...
        <div className="text-left mb-6 space-y-1 text-sm">
          <p>
            ⏳ <strong>Time:</strong> {data.hours}h {data.minutes}m
          </p>
          <p>
            💳 <strong>Amount to pay:</strong> {data.price} EUR
          </p>
          <p>
            📛 <strong>License Number:</strong> {data.licensePlate}
          </p>
        </div>
              ...
              {["debit", "credit"].map((type) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors
                    ${
                      cardType === type
                        ? "text-white border-[#0172f2]" // selected
                        : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"
                    }`}
                  style={
                    cardType === type ? { backgroundColor: "#0172f2" } : {}
                  }
                >
                  {type.charAt(0).toUpperCase() + type.slice(1)}
                </button>
